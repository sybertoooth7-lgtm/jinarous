...
- `CORS_ORIGIN` — the URL your frontend runs on (e.g. `http://localhost:5173` for dev, your real domain in production)
...
- **Local dev**: nothing to do — `vite.config.ts` already proxies `/api/*` to `http://localhost:4000`. Just run the backend and `npm run dev` in `app/` side by side.
