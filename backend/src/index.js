...
// Nudge, not a hard failure: a deployment with zero admin users isn't
// broken, just not usable yet (the /admin login has nobody who can log
// in). Easy to forget this one-time step on a fresh deploy, so surface it
// loudly in the boot log rather than have it show up as a confusing "why
// can't I log in?" moment later.
const adminCount = db.prepare('SELECT COUNT(*) AS c FROM admin_users').get().c;
if (adminCount === 0) {
  logger.warn(
    'No admin users exist yet - the /admin dashboard has no way to log in. Run `npm run create-admin` ' +
    '(once, on this host/environment) to create one.'
  );
}
